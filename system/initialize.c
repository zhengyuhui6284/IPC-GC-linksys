...
static	void	sysinit(void)
{
	...
	for (i = 0; i < NPROC; i++) {
		...
		prptr->prprio = 0;
		prptr->tobe_send = newqueue();
		prptr->prasync = FALSE;
	
		/* lab3 - 3 init memb */
		prptr->prhasmem = FALSE;
		prptr->prmemlist = (mb_ent*)getmem(sizeof(mb_ent));
	...
	/* lab3 - 2 init callback entry */
	for (i = 0; i < NPROC; i++) {
		callback_ent[i] = getmem(sizeof(cbent));
		cbptr = (cbent*)callback_ent[i];	
		cbptr->async_msg = NULL;
		cbptr->callback_func = NULL;
	}
